...
//             		int lastPage = (int)Math.ceil((userCnt*1.0)/pageSize);
            	...
            	<nav style="text-align:center;"> 
				  <ul class="pagination">
            		...
            		<%	}%>
            		<c:forEach begin="1" end="${userCnt/pageSize + (userCnt%pageSize > 0 ? 1 : 0)}" var="i">
            			<li <c:if test="${i == page}">
            					class="active"
            				</c:if>
				    ...
				    <c:choose>
				    	<c:when test="${page == (userCnt/pageSize + (userCnt%pageSize > 0 ? 1 : 0))}">
				    		<li class="disabled">
	    						<a aria-label="Next">
				        			<span aria-hidden="true">&raquo;</span>
				      			</a>
				    	...
				    	<c:otherwise>
				    		<li>
	    						<a href="${pageContext.servletContext.contextPath}/userPagingList?page=${userCnt/pageSize + (userCnt%pageSize > 0 ? 1 : 0)}" aria-label="Next">
				        			<span aria-hidden="true">&raquo;</span>
				      			</a>
				      		</li>
				    	</c:otherwise>
